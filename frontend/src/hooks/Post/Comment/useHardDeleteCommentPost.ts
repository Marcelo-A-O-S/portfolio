...
import { hardRemovePostComment } from "@/services/client/post-services";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { AxiosError, AxiosResponse } from "axios";
import { toast } from "sonner";

export function useHardDeleteCommentPost(){
    const queryClient = useQueryClient();
    return useMutation<AxiosResponse<ApiResponse>, AxiosError<ApiResponse>, DeleteProps<CommentSchema>>({
        mutationFn: ({ id, data }) => hardRemovePostComment(id, data),
        onSuccess: (response) => {
