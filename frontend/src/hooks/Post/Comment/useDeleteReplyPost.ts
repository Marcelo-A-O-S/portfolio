...
import { deletePostReply } from "@/services/server/post-services";
import { toast } from "sonner";

export function useDeleteReplyPost(){
    const queryClient = useQueryClient();
    return useMutation<AxiosResponse<ApiResponse>, AxiosError<ApiResponse>, DeleteProps<ReplyPostSchema>>({
        mutationFn: ({id, data}) => deletePostReply(data.ownerId, id, data.reply),
        onSuccess: (response)=>{
