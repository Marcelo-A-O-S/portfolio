...
namespace PostService.Application.Validators
{
    public class ValidationServices : IValidationServices
    {
        ...
        public async Task ValidateLanguageExists(Guid languageId)
        {
            var languageCache = await this.languageCacheServices.GetLanguageCache(CacheKeys.LanguageExists(languageId));
            if (languageCache == null)
            {
                var exists = await this.languageServices.Exists(languageId);
                if(!exists)
                    throw new NotFoundException("Linguagem não encontrado");
                await this.languageCacheServices.AddLanguageCache(CacheKeys.LanguageExists(languageId), languageId);
            }
        }
